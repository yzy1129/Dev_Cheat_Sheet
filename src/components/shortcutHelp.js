...
export function showShortcutHelp() {
  ...
  overlay.innerHTML = `
    <div class="modal-content-enter bg-white dark:bg-gray-900 rounded-2xl shadow-2xl shadow-black/20 max-w-xs w-full p-6">
      <h2 class="text-base font-bold mb-4">${t('shortcuts.title')}</h2>
      <div class="space-y-3 text-sm">
        ${shortcut('/', t('shortcuts.focusSearch'))}
        ${shortcut('Ctrl+K', t('shortcuts.globalSearch'))}
        ${shortcut('Esc', t('shortcuts.clearOrClose'))}
        ${shortcut('?', t('shortcuts.showHelp'))}
      </div>
      <button class="mt-5 w-full py-2.5 bg-gray-100/80 dark:bg-white/5 rounded-xl text-xs font-medium hover:bg-gray-200/80 dark:hover:bg-white/10 transition-colors" id="close-help">${t('shortcuts.close')}</button>
    </div>
  `
